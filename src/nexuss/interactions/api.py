...
def register_interaction_routes(
    ...
    *,
    ...
    def create_interaction(
        ...
        try:
            progress.start(body.request_id, session_id, body.conversation_id)
        except PermissionError:
            raise HTTPException(404, "Request not found") from None
        except RuntimeError:
            raise HTTPException(409, "Request is already running or progress capacity is full") from None

        try:
            with reporting(lambda kind, state, detail: progress.append(body.request_id, kind, state, detail)):
            ...
            progress.finish(body.request_id, "failed")
            raise HTTPException(500, "Interaction failed; inspect the retained workflow before retrying") from None

    @app.get("/v1/interactions/progress/{request_id}")
    def get_progress(request_id: UUID, request: Request,
        ...
        if not authenticated:
            raise HTTPException(401, "Authenticated session is required")
        try:
